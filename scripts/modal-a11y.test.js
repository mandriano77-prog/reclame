...
test('legacy modal gets dialog role and labelledby from header', () => {
  const modal = {
    id: 'templateModal',
    dataset: {},
    ...
    querySelector(sel) {
      if (sel.includes('modal-header')) return { id: '', textContent: 'Nuovo Template' };
      return null;
    },
    addEventListener() {}
  };
  ...
  assert.equal(modal.getAttribute('aria-labelledby'), 'templateModalTitle');
});
