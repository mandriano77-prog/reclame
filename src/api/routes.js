...
// Throttle dashboard login brute-force. skipSuccessfulRequests so a valid login
// (2xx) never counts — only failed attempts (401/403/400) consume the budget.
...
registerTemplateRoutes(router, { requireBrandId, isHrBrand, validateTemplateBackPayload, syncGoogleWalletClassForTemplate });
...
router.delete('/brands/:brand_id/members/:member_id', async (req, res) => {
  try {
    if (!requireWriteAccess(req, res)) return;
    const { brand_id, member_id } = req.params;
    if (!requireBrandId(req, res, brand_id)) return;
    const brand = await getBrand(brand_id);
    if (!brand || !isHrBrand(brand, req)) {
      return res.status(400).json({ error: 'Eliminazione dipendente disponibile solo per brand HR' });
    }
